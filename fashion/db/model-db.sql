...
create table items (
    ...
    user_id varchar(128) references users(iid) ON DELETE CASCADE
);
...
create table purchases (
    iid varchar(128) primary key,
    buyer_id varchar(128) references users(iid) ON DELETE CASCADE,
    seller_id varchar(128) references users(iid) ON DELETE CASCADE,
    delivery_id varchar(128) references deliveries(iid) ON DELETE CASCADE,
...
create table request_lines (
    item_id varchar(128) references items(iid) ON DELETE CASCADE,
    size varchar(128) not null,
    purchase_id varchar(128) references purchases(iid) ON DELETE CASCADE,
    quantity int not null
);
...
create table booked_items (
    iid varchar(128) primary key,
    rental_item_id varchar(128) references rental_items(iid) ON DELETE CASCADE,
